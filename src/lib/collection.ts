...
export class RedisCollection<V, ReturnType = Serialized<V>> {
	...
	public constructor(options: RedisCollectionOptions<V, ReturnType>, entries?: readonly (readonly [string, V])[] | null);
	...
	public async forEach<T>(fn: (this: T, value: ReturnType, key: string, collection: this) => void, thisArg: T): Promise<void>;
	public async forEach(fn: (value: ReturnType, key: string, collection: this) => void, thisArg?: unknown): Promise<void> {
	...
	public async first(amount?: number): Promise<ReturnType | ReturnType[] | undefined> {
		if (amount === undefined) return (await this.values().next()).value;
	...
	public async firstKey(amount?: number): Promise<string[] | string | undefined> {
		if (amount === undefined) return (await this.keys().next()).value;
	...
	public async last(amount?: number): Promise<ReturnType | ReturnType[] | undefined> {
		...
		if (amount === undefined) return values[values.length - 1];
	...
	public async lastKey(amount?: number): Promise<string[] | string | undefined> {
		...
		if (amount === undefined) return keys[keys.length - 1];
	...
	public async random(amount?: number): Promise<ReturnType | ReturnType[] | undefined> {
		const values = await this.valuesArray();
		if (amount === undefined) return values[Math.floor(Math.random() * values.length)];
	...
	public async randomKey(amount?: number): Promise<string[] | string | undefined> {
		const keys = await this.keysArray();
		if (amount === undefined) return keys[Math.floor(Math.random() * keys.length)];
		return Array.from({ length: Math.min(amount, keys.length) }, (): string => keys.splice(Math.floor(Math.random() * keys.length), 1)[0]!);
	}
	 ...
	 * ```
	 */
	public async find<V2 extends ReturnType>(fn: (value: ReturnType, key: string, collection: this) => value is V2): Promise<V2 | undefined>;
	...
	public async find(fn: (value: ReturnType, key: string, collection: this) => unknown, thisArg?: unknown): Promise<ReturnType | undefined> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
	 ...
	 * ```
	 */
	public async findKey<K2 extends string>(fn: (value: ReturnType, key: string, collection: this) => key is K2): Promise<K2 | undefined>;
	...
	public async findKey(fn: (value: ReturnType, key: string, collection: this) => unknown, thisArg?: unknown): Promise<string | undefined> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
		for await (const [key, val] of this) {
	...
	public async sweep<T>(fn: (this: T, value: ReturnType, key: string, collection: this) => unknown, thisArg: T): Promise<number>;
	public async sweep(fn: (value: ReturnType, key: string, collection: this) => unknown, thisArg?: unknown): Promise<number> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
	 ...
	 * ```
	 */
	public async filter(fn: (value: ReturnType, key: string, collection: this) => unknown): Promise<Collection<string, ReturnType>>;
	...
	public async partition(
		...
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
		const results: [Collection<string, ReturnType>, Collection<string, ReturnType>] = [new Collection(), new Collection()];
		for await (const [key, val] of this) {
			if (fn(val, key, this)) {
	 ...
	 * ```
	 */
	public async flatMap<T>(fn: (value: ReturnType, key: string, collection: this) => Collection<string, T>): Promise<Collection<string, T>>;
	...
	public async map<This, T>(fn: (this: This, value: ReturnType, key: string, collection: this) => T, thisArg: This): Promise<T[]>;
	 ...
	 * ```
	 */
	public async mapValues<T>(fn: (value: ReturnType, key: string, collection: this) => T): Promise<Collection<string, T>>;
	public async mapValues<This, T>(
		fn: (this: This, value: ReturnType, key: string, collection: this) => T,
		thisArg: This
	): Promise<Collection<string, T>>;
	public async mapValues<T>(fn: (value: ReturnType, key: string, collection: this) => T, thisArg?: unknown): Promise<Collection<string, T>> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
	...
	public async some<T>(fn: (this: T, value: ReturnType, key: string, collection: this) => unknown, thisArg: T): Promise<boolean>;
	public async some(fn: (value: ReturnType, key: string, collection: this) => unknown, thisArg?: unknown): Promise<boolean> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
		for await (const [key, val] of this) {
	...
	public async every<V2 extends ReturnType>(fn: (value: ReturnType, key: string, collection: this) => value is V2): Promise<boolean>;
	public async every(fn: (value: ReturnType, key: string, collection: this) => unknown): Promise<boolean>;
	...
	public async every<This>(fn: (this: This, value: ReturnType, key: string, collection: this) => unknown, thisArg: This): Promise<boolean>;
	public async every(fn: (value: ReturnType, key: string, collection: this) => unknown, thisArg?: unknown): Promise<boolean> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		if (thisArg !== undefined) fn = fn.bind(thisArg);
		for await (const [key, val] of this) {
	 ...
	 * ```
	 */
	public async reduce<T>(fn: (accumulator: T, value: ReturnType, key: string, collection: this) => T, initialValue?: T): Promise<T> {
		if (typeof fn !== 'function') throw new TypeError(`${fn} is not a function`);
		let accumulator!: T;
	...
	public async each<T>(fn: (this: T, value: ReturnType, key: string, collection: this) => void, thisArg: T): Promise<this>;
	 ...
	 */
	public async intersect<T>(other: ReadonlyCollection<string, T> | ReadonlyRedisCollection<any, T>): Promise<Collection<string, T>> {
		const coll = new Collection<string, T>();
	 ...
	 */
	public async subtract<T>(other: ReadonlyCollection<string, T> | ReadonlyRedisCollection<any, T>): Promise<Collection<string, ReturnType | T>> {
		const coll = new Collection<string, ReturnType | T>();
		for await (const [k, v] of this) {
			if (!(await other.has(k)) || !Object.is(v, await other.get(k))) {
				coll.set(k, v);
			}
		}
